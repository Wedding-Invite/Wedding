...
$(document).ready(function() {
	...
	if( $('#countdown').length ) {
		...
		var countDownDate = Date.parse('19 Jan 2019 18:00:00');
...
function sendConfirmation(obj){
	//alert(obj);
	var messageToM = "Hello Dhana,\n\n" + encodeURIComponent($('#name').val()) + " would like to register for accommodation. Checkout the details.\n" +
				"Email ID : " + encodeURIComponent($('#email').val()) +
				"\nNo. Of Guest : " + encodeURIComponent($('#guest option:selected').val()) +
				"\nAttending Event : " + encodeURIComponent($('#events option:selected').val()) + 
				"\nMessage : " + encodeURIComponent($('#message').val());
	//alert(messageToM);
	var messageToF = "Hello Kavi,\n\n" + encodeURIComponent($('#name').val()) + " would like to register for accommodation. Checkout the details.\n" +
				"Email ID : " + encodeURIComponent($('#email').val()) +
				"\nNo. Of Guest : " + encodeURIComponent($('#guest option:selected').val()) +
				"\nAttending Event : " + encodeURIComponent($('#events option:selected').val()) + 
				"\nMessage : " + encodeURIComponent($('#message').val());
	//alert(messageToF);
	var whatsappNumberM = encodeURIComponent($('#whatsappNumberM').val())
	var whatsappNumberF = encodeURIComponent($('#whatsappNumberF').val())
	//alert(whatsappNumberF + "\t" + whatsappNumberM)
	var sendTo = encodeURIComponent($('#friend option:selected').val());
	var url = "https://wa.me/#NUMBER#/?text=#MESSAGE#";
	
	if("M" == sendTo){
		url = url.replace("#NUMBER#", whatsappNumberM);
		url = url.replace("#MESSAGE#", encodeURIComponent(messageToM));
	}else{
		url = url.replace("#NUMBER#", whatsappNumberF);
		url = url.replace("#MESSAGE#", encodeURIComponent(messageToF));		
	}
	//alert(url);
	//return false;
	...
	var urlAttendee = "https://script.google.com/macros/s/AKfycbzLvXbDi_jeaU8nlAbdx4xh1JIQ-tZ9j4If14L4AFr4SvVKFx4/exec?callback=?";
	
	var requestAttendee = jQuery.ajax({
		crossDomain: true,
		url: urlAttendee + formData,
		method: "GET",
		dataType: "jsonp"
	});*/
